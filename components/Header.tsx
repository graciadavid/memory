...
export default function Header() {
 ...
 useEffect(() => {
   ...
   // World rank = position by number of games played
   supabase.from('profiles').select('player_name', { count: 'exact', head: true })
     .then(({ count }: any) => {
       if (!count) return
       // Simple rank based on profile position
       supabase.from('profiles').select('player_name').eq('player_name', p.name).single()
         .then(() => {
           // Get rank by streak desc
           supabase.from('profiles').select('player_name', { count: 'exact', head: true })
             .gt('streak', streak)
             .then(({ count: better }: any) => setWorldRank((better || 0) + 1))
         })
     })
 }, [streak])
     ...
     <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 60 }}>
       ...
       {profile && worldRank && (
         <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
           ...
           <span style={{ fontSize: 11, fontWeight: 800, color: 'rgba(255,255,255,0.4)' }}>#{worldRank}</span>
         </div>
       )}
     </div>
