...
const COUNTRIES = [
  ...
  { name: 'Mozambique', code: 'mz', population: 32000000 },
  { name: 'Ghana', code: 'gh', population: 32000000 },
  { name: 'Yemen', code: 'ye', population: 33000000 },
  { name: 'Nepal', code: 'np', population: 30000000 },
  { name: 'Australia', code: 'au', population: 26000000 },
  { name: 'Cameroon', code: 'cm', population: 27000000 },
  ...
  { name: 'Guatemala', code: 'gt', population: 17000000 },
  { name: 'Cambodia', code: 'kh', population: 17000000 },
  { name: 'Zimbabwe', code: 'zw', population: 16000000 },
  { name: 'Senegal', code: 'sn', population: 17000000 },
  { name: 'Chad', code: 'td', population: 17000000 },
  ...
  { name: 'Jordan', code: 'jo', population: 10000000 },
  { name: 'UAE', code: 'ae', population: 9000000 },
  { name: 'Honduras', code: 'hn', population: 10000000 },
  { name: 'Azerbaijan', code: 'az', population: 10000000 },
  ...
  { name: 'Luxembourg', code: 'lu', population: 660000 },
  { name: 'Malta', code: 'mt', population: 520000 },
]
...
function getPair(used: Set<number>) {
  const available = COUNTRIES.filter((_, i) => !used.has(i))
  if (available.length < 2) return null
  const shuffle = [...available].sort(() => Math.random() - 0.5)
  const a = COUNTRIES.indexOf(shuffle[0])
  const b = COUNTRIES.indexOf(shuffle[1])
  return { a: COUNTRIES[a], b: COUNTRIES[b], ai: a, bi: b }
}
        ...
        {phase === 'playing' && pair && (
          <>
            ...
            <button onClick={() => answer('a')} disabled={!!selected} style={{
              width: '100%', borderRadius: 20, border: `3px solid ${selected === 'a' ? (correct ? '#2E7D32' : '#B71C1C') : selected ? `${BROWN}20` : COLOR}`,
            ...
            <button onClick={() => answer('b')} disabled={!!selected} style={{
              width: '100%', borderRadius: 20, border: `3px solid ${selected === 'b' ? (correct ? '#2E7D32' : '#B71C1C') : selected ? `${BROWN}20` : COLOR}`,
