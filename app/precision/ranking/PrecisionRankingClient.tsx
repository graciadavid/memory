...
export default function PrecisionRankingClient({ scores }: { scores: { name: string, diff: number, created_at: string }[] }) {
  ...
  const share = async (position: number, diff: number) => {
    const text = `⏱ I'm #${position} in MemGenius Precision with ${(diff/1000).toFixed(3)}s off!\nCan you beat me? 👉 https://memgenius.com/precision`
