...
export default function CountriesPage() {
 ...
 if (phase === 'playing') return (
   <main style={{ minHeight:'100dvh', background:'#1C1C1E', fontFamily:'var(--font-nunito), sans-serif', maxWidth:430, margin:'0 auto', display:'flex', flexDirection:'column', padding:'24px 20px 100px', overflowY:'auto' }}>
   ...
   <main style={{ minHeight:'100dvh', background:bgResult, fontFamily:'var(--font-nunito), sans-serif', maxWidth:430, margin:'0 auto', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'32px 24px 100px', gap:20, overflowY:'auto' }}>
     ...
     {!profile?.name && !saved && (
       <AuthModal onSuccess={async (playerName) => {
           await supabase.from('shape_scores').insert({player_name: playerName, level: score})
           setSaved(true)
