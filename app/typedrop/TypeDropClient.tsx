...
export default function TypeDropClient() {
  ...
  const spawnWord = useCallback((currentScore: number) => {
    ...
    const animate = () => {
      ...
      if (progress >= 1) {
        ...
        // Get rank
        supabase.from('typedrop_scores').select('*', { count: 'exact', head: true }).gt('score', finalScore)
          .then(({ count }: any) => {
  ...
  if (phase === 'over') return (
    <main style={{ minHeight:'100dvh', background:'#1C1C1E', fontFamily:'var(--font-nunito), sans-serif', maxWidth:430, margin:'0 auto', padding:'32px 20px 100px', textAlign:'center' }}>
      ...
      <div style={{ fontSize:13, color:'rgba(255,255,255,0.4)', marginBottom:16 }}>The word was <span style={{ color:'#fff', fontWeight:900 }}>{word}</span></div>
      <div style={{ fontSize:72, fontWeight:900, color:GOLD, marginBottom:4 }}>{score}</div>
      <div style={{ fontSize:13, color:'rgba(255,255,255,0.4)', fontWeight:700, marginBottom:8 }}>words typed</div>
      {worldRank && <div style={{ fontSize:16, color:GOLD, fontWeight:900, marginBottom:16 }}>#{worldRank} in the world</div>}
      {saved && <div style={{ fontSize:13, color:'#69F0AE', fontWeight:800, marginBottom:16 }}>✓ Saved!</div>}
      ...
      <div style={{ flex:1, position:'relative', overflow:'hidden', background:'rgba(0,0,0,0.2)', borderRadius:'12px 12px 0 0', margin:'0 8px' }}>
        ...
        <div style={{ position:'absolute', left:'50%', transform:'translateX(-50%)', top:`${posY}%`, display:'flex', gap:3, whiteSpace:'nowrap' }}>
          {word.split('').map((letter, i) => (
            <span key={i} style={{ fontSize:38, fontWeight:900, color:letterColors[i], textShadow: danger ? '0 0 20px rgba(255,82,82,0.9)' : 'none', transition:'color 0.08s' }}>{letter}</span>
          ))}
        </div>
          ...
          <div key={i} style={{ display:'flex', gap:4, justifyContent:'center', marginBottom:4 }}>
            {row.map(k => (
              <button key={k} onPointerDown={(e) => { e.preventDefault(); handleKey(k) }}
                style={{ height:44, minWidth: k === '⌫' || k === '↵' ? 48 : 32, borderRadius:8, border:'none', background:'rgba(255,255,255,0.12)', color:'#fff', fontSize: k.length > 1 ? 13 : 17, fontWeight:900, cursor:'pointer', fontFamily:'inherit', padding:'0 4px', userSelect:'none' }}>
                {k}
              </button>
            ))}
          </div>
