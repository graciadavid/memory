...
export default function SequenceClient() {
 ...
 const handlePress = useCallback(async (colorId: number) => {
   ...
   if (newInput[idx] !== sequence[idx]) {
     ...
     if (profile?.name && finalLevel > 0) { await supabase.from("sequence_scores").insert({ player_name: profile.name, level: finalLevel }); supabase.rpc("update_streak", { p_player_name: profile.name }) }
 return
     return
   }
