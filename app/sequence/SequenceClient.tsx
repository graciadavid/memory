...
const COLORS = [
  { id: 0, color: '#D32F2F', light: '#FF5252', label: 'Red' },
  { id: 1, color: '#1565C0', light: '#448AFF', label: 'Blue' },
  { id: 2, color: '#2E7D32', light: '#69F0AE', label: 'Green' },
  { id: 3, color: '#F57F17', light: '#FFD740', label: 'Yellow' },
]
...
export default function SequenceClient() {
  ...
  const loadData = useCallback(async () => {
    const { data } = await supabase.from('sequence_scores')
  ...
  const handlePress = useCallback(async (colorId: number) => {
    ...
    if (newInput[idx] !== sequence[idx]) {
      ...
      const { count } = await supabase.from('sequence_scores')
        .select('player_name', { count: 'exact', head: true }).gt('level', finalLevel)
      setWorldRank((count ?? 0) + 1)
      if (profile?.name && finalLevel > 0) {
        await supabase.from('sequence_scores').insert({ player_name: profile.name, level: finalLevel })
      }
      return
    }
  ...
  if (phase === 'rules') return (
    <GameRulesScreen
      icon="sequence.png"
      title="Simon Says"
      subtitle="Repeat the color pattern"
  ...
  if (phase === 'result') return (
    <GameResultScreen
      ...
      resultColor={finalLevel >= 15 ? '#00C853' : finalLevel >= 8 ? GOLD : '#D32F2F'}
      background={finalLevel >= 15 ? '#0D3320' : finalLevel >= 8 ? '#2D1A00' : '#1A0000'}
      worldRank={worldRank}
  ...
  return (
    <main style={{ height:'100dvh', background:'#1A1A1A', fontFamily:'var(--font-nunito),sans-serif', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:24, paddingBottom:80 }}>
      <div style={{ fontSize:13, fontWeight:800, color:'rgba(255,255,255,0.3)', letterSpacing:2 }}>
        {phase === 'show' ? 'WATCH' : `LEVEL ${level} — REPEAT`}
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, width:280 }}>
        {COLORS.map(c => (
          <button key={c.id} onPointerDown={() => handlePress(c.id)}
            disabled={phase === 'show'}
            style={{
              height:120, borderRadius:20, border:'none', cursor: phase === 'input' ? 'pointer' : 'default',
              background: activeColor === c.id ? c.light : c.color,
              boxShadow: activeColor === c.id ? `0 0 30px ${c.light}` : 'none',
      ...
      <div style={{ fontSize:13, color:'rgba(255,255,255,0.2)', fontWeight:700 }}>
        {phase === 'show' ? 'Memorize the sequence' : `${userInput.length} / ${sequence.length}`}
      </div>
    </main>
  )
}
