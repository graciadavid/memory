...
export default function PokeClient() {
 ...
 const handleSubmit = useCallback(async () => {
   ...
   if (correct) {
     ...
     if (profile?.name && level > 1) { await supabase.from("poke_scores").insert({ player_name: profile.name, level: level - 1 }); supabase.rpc("update_streak", { p_player_name: profile.name }) }
 }
