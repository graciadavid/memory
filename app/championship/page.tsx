        ...
        <div onClick={() => setSelectedWinner(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.85)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
          <div onClick={e => e.stopPropagation()} style={{ width:'100%', maxWidth:380 }}>
            <div style={{ background:'linear-gradient(135deg,#8B6914,#C8960C,#FFD700,#C8960C,#8B6914)', borderRadius:24, padding:'24px', boxShadow:'0 12px 0 rgba(100,70,0,0.5)' }}>
              ...
              <a onClick={() => { const imgUrl = `https://memgenius.com/api/og/championship?name=${encodeURIComponent(selectedWinner.winner_name)}&score=${selectedWinner.score}&game=${encodeURIComponent(selectedWinner.game)}&edition=${String(hallOfFame.findIndex((h:any) => h.id === selectedWinner.id) + 1).padStart(3,"0")}`; if (navigator.share) { navigator.share({ title: "Sunday Brain Championship", text: `${selectedWinner.winner_name} won the Sunday Brain Championship with ${selectedWinner.score}ms on ${selectedWinner.game}! Can you beat them?`, url: imgUrl }) } else { navigator.clipboard.writeText(imgUrl); alert("Link copied!") } }} style={{ textDecoration:"none", display:"block", width:"100%", padding:"14px", borderRadius:12, background:"rgba(0,0,0,0.25)", color:"#000", fontSize:15, fontWeight:900, fontFamily:"var(--font-nunito),sans-serif", textAlign:"center", cursor:"pointer" }}>
                🔗 Share Champion
              </a>
            </div>
