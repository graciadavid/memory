     ...
     <div style={{ textAlign:'center', marginBottom:32 }}>
      ...
      <div style={{ background:'linear-gradient(135deg, #8B6914, #C8960C, #FFD700, #C8960C, #8B6914)', borderRadius:24, padding:'14px 16px', marginBottom:12, boxShadow:'0 5px 0 rgba(100,70,0,0.5)' }}>
       ...
       <div style={{ marginBottom:20 }}>
        ...
        <button onClick={async () => { const res = await supabase.from('events').insert({ event: 'championship_share', player_name: profile?.name || 'anonymous', meta: week?.sunday_date }); console.log('share event:', res); if (navigator.share) { navigator.share({ title:'Sunday Brain Championship', text:'Can you beat the world? This Sunday: ' + gameLabel, url:'https://memgenius.com/championship' }) } else { navigator.clipboard.writeText('https://memgenius.com/championship'); alert('Link copied!') } }} style={{ width:'100%', marginTop:10, padding:'14px', borderRadius:16, border:'none', background:'#2E7D32', color:'#fff', fontSize:14, fontWeight:900, fontFamily:'var(--font-nunito), sans-serif', cursor:'pointer' }}>
          🔗 Share Championship
        </button>
