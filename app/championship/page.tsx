 ...
 return (
   ...
   <main style={{ minHeight:'100dvh', background:'#1C1C1E', fontFamily:'var(--font-nunito), sans-serif', maxWidth:430, margin:'0 auto', padding:'32px 20px 100px' }}>
     ...
     <div style={{ textAlign:'center', marginBottom:32 }}>
      ...
      <div style={{ background:'linear-gradient(135deg, #8B6914, #C8960C, #FFD700, #C8960C, #8B6914)', borderRadius:20, padding:'14px 16px', marginBottom:16, boxShadow:'0 5px 0 rgba(100,70,0,0.5)' }}>
        <div style={{ fontSize:11, fontWeight:800, color:'rgba(0,0,0,0.5)', letterSpacing:2, marginBottom:8 }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:12 }}>
          {[{v:countdown.h,l:'HRS'},{v:countdown.m,l:'MIN'},{v:countdown.s,l:'SEC'}].map(({v,l}) => (
            <div key={l} style={{ flex:1, background:'rgba(0,0,0,0.2)', borderRadius:10, padding:'8px 4px', textAlign:'center' }}>
              <div style={{ fontSize:22, fontWeight:900, color:'#000' }}>{String(v).padStart(2,'0')}</div>
              <div style={{ fontSize:9, fontWeight:800, color:'rgba(0,0,0,0.4)', letterSpacing:2 }}>{l}</div>
            </div>
          ))}
        </div>
        <a href={gameHref} style={{ textDecoration:'none', display:'block', background:'rgba(0,0,0,0.3)', borderRadius:14, padding:'14px', textAlign:'center' }}>
