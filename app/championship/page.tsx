       ...
       <div style={{ background: '#252525', borderRadius: 16, padding: '16px', marginBottom: 12 }}>
         ...
         {ranking.map((r, i) => (
           <div key={r.name} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
             <div style={{ fontSize:12, fontWeight:900, color:i===0?GOLD:'rgba(255,255,255,0.25)', width:20 }}>#{i+1}</div>
         ...
         {hallOfFame.map((h, i) => (
           <div key={h.id} onClick={() => setSelectedWinner(h)}
             ...
             <div style={{ flex:1 }}>
               ...
               <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', fontWeight:700, textTransform:'capitalize' }}>
                 {h.game} · {new Date(h.sunday_date).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}
               </div>
             </div>
           ...
           <div style={{ background:'linear-gradient(135deg,#8B6914,#C8960C,#FFD700,#C8960C,#8B6914)', borderRadius:24, padding:'24px', boxShadow:'0 12px 0 rgba(100,70,0,0.5)' }}>
             ...
             <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:16 }}>
               ...
               <div style={{ background:'rgba(0,0,0,0.15)', borderRadius:10, padding:'10px', textAlign:'center' }}>
                 ...
                 <div style={{ fontSize:12, fontWeight:900, color:'#000' }}>{new Date(selectedWinner.sunday_date).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</div>
               </div>
             </div>
             <div style={{ fontSize:10, fontWeight:700, color:'rgba(0,0,0,0.35)', textAlign:'center', marginBottom:14 }}>{selectedWinner.participants} players competed</div>
             <button onClick={() => {
               if (navigator.share) {
                 navigator.share({ title:'Sunday Brain Championship', text:`${selectedWinner.winner_name} won the Sunday Brain Championship with ${selectedWinner.score}ms on ${selectedWinner.game}. memgenius.com/championship`, url:'https://memgenius.com/championship' })
               } else {
                 navigator.clipboard.writeText(`${selectedWinner.winner_name} won with ${selectedWinner.score}ms. memgenius.com/championship`)
               }
