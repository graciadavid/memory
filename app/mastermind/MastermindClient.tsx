...
export default function MastermindClient() {
 ...
 useEffect(() => {
  ...
  const loadData = async () => {
    const { data } = await supabase.from('mastermind_scores').select('player_name,attempts,time_ms').order('attempts', { ascending: true }).order('time_ms', { ascending: true }).limit(5000)
    if (!data) return
