...
export default function WordlyClient() {
 ...
 const loadData = async () => {
   ...
   if (pName && best[pName]) setMyBest(best[pName].attempts)
 }
 ...
 const saveScore = async () => {
   ...
   await supabase.from('wordle_scores').insert({player_name:name.trim(), attempts:guessesRef.current.length, time_ms:finalTime})
   const {count} = await supabase.from('wordle_scores').select('*',{count:'exact',head:true}).lt('attempts',guessesRef.current.length)
   setWorldRank((count??0)+1)
   setSaving(false); setSaved(true)
   localStorage.setItem('memgenius_profile', JSON.stringify({name:name.trim()}))
   playerNameRef.current = name.trim()
   setMyBest(prev => prev===null || guessesRef.current.length<prev ? guessesRef.current.length : prev)
   setTimeout(() => window.location.reload(), 1500)
 }

 const reset = () => {
   if (timerRef.current) clearInterval(timerRef.current)
   setPhase('rules'); setSaved(false); loadData()
 }
 ...
 const rows = Array.from({length:6}, (_,i) => {
   ...
   if (i === guesses.length && (phase === 'playing' || phase === 'won')) {
