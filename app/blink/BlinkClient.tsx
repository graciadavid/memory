        ...
        {!profile?.name && !saved && won && (
          <div style={{ background:'rgba(0,0,0,0.3)', borderRadius:14, padding:'12px', marginBottom:16 }}>
            ...
            <div style={{ display:'flex', gap:5, justifyContent:'center', marginBottom:8 }}>
              {pin.map((d,i) => (
                <input key={i} id={`pin-blink-${i}`} type="tel" maxLength={1} value={d}
                  onChange={e=>{const v=e.target.value.replace(/\D/,'');const p=[...pin];p[i]=v;setPin(p);if(v&&i<3)(document.getElementById(`pin-blink-${i+1}`) as HTMLInputElement)?.focus()}}
