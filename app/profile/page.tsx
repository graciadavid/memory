...
export default function ProfilePage() {
  ...
  return (
    <main style={{ minHeight:'100dvh', background:'#1C1C1E', fontFamily:'var(--font-nunito), sans-serif', maxWidth:430, margin:'0 auto', paddingBottom:100 }}>
      ...
      {/* Hero */}
      <div style={{ background:'linear-gradient(180deg, #2A2A2E 0%, #1C1C1E 100%)', padding:'40px 24px 32px', textAlign:'center' }}>
        {/* Avatar */}
        <div style={{ position:'relative', display:'inline-block', marginBottom:16 }}>
          <div style={{ width:84, height:84, borderRadius:'50%', background:`linear-gradient(135deg, ${GOLD}, #FF6D00)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:36, fontWeight:900, color:'#fff', boxShadow:`0 8px 24px rgba(200,150,12,0.4)` }}>
            {initial}
          </div>
          {streak > 0 && (
            <div style={{ position:'absolute', bottom:-4, right:-4, background:'#1C1C1E', borderRadius:'50%', padding:4 }}>
              <img src={`${BASE}/streak.png`} style={{ width:28, height:28, objectFit:'contain', display:'block' }} />
            </div>
          )}
        </div>

        <div style={{ fontSize:26, fontWeight:900, color:'#fff', marginBottom:4 }}>{profile?.name ?? 'Guest'}</div>

        {streak > 0 && (
          <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(255,109,0,0.15)', borderRadius:20, padding:'6px 14px', marginBottom:16 }}>
            <img src={`${BASE}/streak.png`} style={{ width:18, height:18, objectFit:'contain' }} />
            <span style={{ fontSize:15, fontWeight:900, color:'#FF6D00' }}>{streak} day streak</span>
          </div>
        )}

        {/* PIN */}
        <button onClick={() => setEditingPin(!editingPin)} style={{ background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:'8px 18px', color:'rgba(255,255,255,0.5)', fontSize:13, fontWeight:800, cursor:'pointer', fontFamily:'inherit' }}>
        ...
        {CATS.map(cat => (
          <div key={cat.label} style={{ marginBottom:24 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
              ...
              <div style={{ fontSize:13, fontWeight:900, color:cat.color, letterSpacing:0.5, textTransform:'uppercase' }}>{cat.label}</div>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {cat.games.map(g => (
                <div key={g.label} style={{ background:'rgba(255,255,255,0.04)', borderRadius:14, padding:'12px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', border:'1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ fontSize:14, fontWeight:800, color:'rgba(255,255,255,0.6)' }}>{g.label}</div>
                  <div style={{ fontSize:14, fontWeight:900, color: g.value ? '#fff' : 'rgba(255,255,255,0.2)' }}>{g.value ?? '—'}</div>
                </div>
              ))}
            </div>
          </div>
